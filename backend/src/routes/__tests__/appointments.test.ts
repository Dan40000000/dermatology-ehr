...
describe("Appointments routes", () => {
  ...
  it("POST /appointments/:id/status creates no-show fee bill for overdue appointment", async () => {
    ...
    queryMock
      ...
      .mockResolvedValueOnce({ rows: [], rowCount: 0 }) // existing no-show fee lookup
      .mockResolvedValueOnce({ rows: [] }) // insert bill
      .mockResolvedValueOnce({ rows: [] }) // insert bill line item
      .mockResolvedValueOnce({ rows: [] }) // update appointment
      .mockResolvedValueOnce({ rows: [] }) // insert history
    ...
    const res = await request(app)
      .post("/appointments/appt-1/status")
      .send({ status: "no_show", reason: "Patient did not arrive after reminder call." });

    expect(res.status).toBe(200);
    expect(res.body.noShowFeeBillId).toBeTruthy();
    const insertBillCall = queryMock.mock.calls.find((call) => String(call[0]).includes("insert into bills"));
    const insertLineItemCall = queryMock.mock.calls.find((call) => String(call[0]).includes("insert into bill_line_items"));
    expect(insertBillCall).toBeTruthy();
    expect(insertLineItemCall).toBeTruthy();
  ...
  it("POST /appointments/:id/status rejects no-show before grace window", async () => {
    const recentStart = new Date(Date.now() - 5 * 60 * 1000).toISOString();
    queryMock
      .mockResolvedValueOnce({ rows: [] }) // BEGIN
      .mockResolvedValueOnce({
        rows: [{ patient_id: "patient-1", scheduled_start: recentStart, status: "scheduled" }],
        rowCount: 1,
      }) // select appointment
      .mockResolvedValueOnce({ rows: [] }); // ROLLBACK
    ...
    const res = await request(app)
      .post("/appointments/appt-1/status")
      .send({ status: "no_show", reason: "Patient has not arrived." });

    expect(res.status).toBe(400);
    expect(res.body.error).toContain("No-show can be confirmed after");
    const updateCall = queryMock.mock.calls.find((call) => String(call[0]).includes("update appointments set status"));
