...
patientsRouter.get("/:id", requireAuth, requireModuleAccess("patients"), async (req: AuthedRequest, res) => {
  ...
  try {
    const result = await pool.query(
      `select p.id,
              nullif(to_jsonb(p)->>'first_name', '') as "firstName",
              nullif(to_jsonb(p)->>'last_name', '') as "lastName",
              nullif(to_jsonb(p)->>'dob', '') as dob,
              nullif(to_jsonb(p)->>'dob', '') as "dateOfBirth",
              nullif(to_jsonb(p)->>'mrn', '') as mrn,
              nullif(to_jsonb(p)->>'phone', '') as phone,
              nullif(to_jsonb(p)->>'email', '') as email,
              nullif(to_jsonb(p)->>'address', '') as address,
              ...
              nullif(to_jsonb(p)->>'account_number', '') as "accountNumber",
              ${canViewSsn ? `nullif(to_jsonb(p)->>'ssn_last4', '') as "ssn"` : `null::text as "ssn"`},
              nullif(to_jsonb(p)->>'emergency_contact_name', '') as "emergencyContactName",
              nullif(to_jsonb(p)->>'emergency_contact_relationship', '') as "emergencyContactRelationship",
              nullif(to_jsonb(p)->>'emergency_contact_phone', '') as "emergencyContactPhone",
              nullif(to_jsonb(p)->>'pharmacy_id', '') as "pharmacyId",
              nullif(to_jsonb(p)->>'pharmacy_ncpdp', '') as "pharmacyNcpdp",
  ...
  try {
    ...
    let query = `
      SELECT p.*,
             ...
             NULLIF(to_jsonb(p)->>'delivery_method', '') as "deliveryMethod",
             NULLIF(to_jsonb(p)->>'delivery_status', '') as "deliveryStatus",
             NULLIF(to_jsonb(p)->>'delivery_notes', '') as "deliveryNotes",
             NULLIF(to_jsonb(p)->>'documented_at', '') as "documentedAt",
             NULLIF(to_jsonb(p)->>'last_printed_at', '') as "lastPrintedAt",
             NULLIF(to_jsonb(p)->>'manually_filled_at', '') as "manuallyFilledAt",
    ...
    const prescriptions = result.rows.map((row: Record<string, any>) => ({
      ...
      deliveryNotes: row.deliveryNotes ?? row.delivery_notes ?? null,
      documentedAt: row.documentedAt ?? row.documented_at ?? null,
      lastPrintedAt: row.lastPrintedAt ?? row.last_printed_at ?? null,
      manuallyFilledAt: row.manuallyFilledAt ?? row.manually_filled_at ?? null,
