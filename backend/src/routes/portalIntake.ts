    ...
    if (kioskConsentColumns.has("consent_form_id")) {
      ...
      const kioskResult = await pool.query(
        `SELECT
           ...
           COALESCE(kpc.form_version, '1.0') as "version",
           kpc.signed_at as "signedAt",
           true as "isValid"
         FROM kiosk_patient_consents kpc
         INNER JOIN consent_forms cf ON kpc.consent_form_id = cf.id
         WHERE kpc.patient_id = $1
           AND kpc.tenant_id = $2
         ORDER BY kpc.signed_at DESC`,
        [patientId, tenantId],
      );
      return kioskResult.rows.map((row) => ({ ...row, source: "kiosk" as const }));
    }
