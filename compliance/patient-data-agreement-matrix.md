...
| Railway synthetic-only pilot | No Railway BAA is required while this environment is contractually and operationally restricted to synthetic test data. An executed Railway BAA would be required before any scope change to PHI. | Platform owner designated Railway as synthetic-only and AWS as the future live-PHI environment on 2026-08-29. No Railway patient tenant may be activated. |
...
| OpenAI API receiving clinical text, images, audio, or identifiers | Executed OpenAI Business Associate and Healthcare Addendum, exact organization provisioning for the applicable HIPAA data-control path, and eligible endpoint/model evidence | Request sent 2026-08-29 for the exact API organization; agreement and provisioning remain pending. `OPENAI_BAA_ENABLED` is unset, so PHI-capable egress remains blocked. OpenAI raw audio remains separately disabled because AWS HealthScribe is the approved scribe path. |
...
For OpenAI specifically, populate `OPENAI_BAA_APPROVED_ENDPOINTS` and `OPENAI_BAA_APPROVED_MODELS` only from the executed agreement and provisioning confirmation. Keep `OPENAI_RAW_AUDIO_ALLOWED=false` unless OpenAI separately confirms the audio-transcription endpoint and the exact organization has the required data-control configuration. The planned production scribe uses AWS HealthScribe, so OpenAI raw audio is not required for launch.
